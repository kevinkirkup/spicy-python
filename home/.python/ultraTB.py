...
class ColorTB:
    def __call__(self, etype, value, tb):
        print(Colors.toplineColor + '-'*60 + Colors.Normal, file=sys.stderr)
        if tb:
            print('Traceback %s(most recent call last)%s:' % \
                                (Colors.normalEm, Colors.Normal), file=sys.stderr)
        ...
        for line in lines[:-1]:
            print(" "+line, end=' ', file=sys.stderr)
        print(lines[-1], end=' ', file=sys.stderr)
